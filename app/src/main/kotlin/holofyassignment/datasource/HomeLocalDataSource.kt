package holofyassignment.datasource
...
import holofyassignment.models.HomeResponseContainer
...
class HomeLocalDataSource @Inject constructor(val application: Application,
    ...
    fun getData(): ArrayList<HomeDataObject> {
        val response = gson.fromJson<HomeResponseContainer>(responseString, HomeResponseContainer::class.java)
        return if (response.categories?.elementAtOrNull(0)?.videos?.isNotEmpty() == true)
            response.categories?.elementAtOrNull(0)?.videos!! else ArrayList()
    }
}